...
void updateBlynkStatus() {
    ...
    bool vocDataFresh = (timeSinceLastVOC < 90000); // Increased to 90 seconds to match new 30s interval
...
void checkVOCLevels() {
    ...
    // VOC data is now received via HTTP API from main ESP32 to virtual pin V14
    // Check if we have recent VOC data (within last 90 seconds)
    unsigned long timeSinceLastVOC = millis() - vocMonitor.lastCheck;
    if (timeSinceLastVOC > 90000) { // Increased to 90 seconds to match new 30s interval
        Serial.println("Warning: No recent VOC data received from main ESP32 via HTTP API");
        return;
    }
    
    // VOC threshold checking is now handled in BLYNK_WRITE(V14)
    // This function just monitors the data freshness
    Serial.print("VOC auto-trigger active. Current level: ");
...
// Virtual Pin V14 - VOC Data from Main ESP32 (HTTP API)
BLYNK_WRITE(V14) {
    ...
    if (vocMonitor.autoEnabled && vocValue > vocMonitor.threshold && !pumpOp.enabled) {
